<script lang="ts">
	...
	async function downloadEpub(content: string, filename: string) {
		...
		<div class="mt-6 bg-white rounded-lg border border-gray-200 p-6">
			...
			<div class="flex gap-2 mb-4">
				{#if activeTab === 'translated-only' && translatedOnlyMarkdown}
					...
					<button
						data-testid="download-epub-translated"
						onclick={() => downloadEpub(translatedOnlyMarkdown, `${baseName}-${langCode}.md`)}
						class="px-3 py-1.5 text-sm bg-emerald-100 hover:bg-emerald-200 text-emerald-700 rounded-lg transition-colors flex items-center gap-1.5"
					>
						<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					...
					<button
						data-testid="download-pdf-translated"
						onclick={() => downloadPdf(translatedOnlyMarkdown, `${baseName}-${langCode}.md`)}
						class="px-3 py-1.5 text-sm bg-red-100 hover:bg-red-200 text-red-700 rounded-lg transition-colors flex items-center gap-1.5"
					>
						<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
				...
				{:else if activeTab === 'bilingual' && bilingualMarkdown}
					...
					<button
						data-testid="download-epub-bilingual"
						onclick={() => downloadEpub(bilingualMarkdown, `${baseName}-bilingual.md`)}
						class="px-3 py-1.5 text-sm bg-emerald-100 hover:bg-emerald-200 text-emerald-700 rounded-lg transition-colors flex items-center gap-1.5"
					>
						<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
					...
					<button
						data-testid="download-pdf-bilingual"
						onclick={() => downloadPdf(bilingualMarkdown, `${baseName}-bilingual.md`)}
						class="px-3 py-1.5 text-sm bg-red-100 hover:bg-red-200 text-red-700 rounded-lg transition-colors flex items-center gap-1.5"
					>
						<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
